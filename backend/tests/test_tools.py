...
def test_tool_registry_registration():
    ...
    assert (
        categories["agent"] == 6
    ), f"Expected 6 Agent tools, got {categories['agent']}"
    assert (
        categories["local"] == 7
    ), f"Expected 7 Local tools, got {categories['local']}"
...
def test_tool_search():
    ...
    assert (
        len(file_tools) >= 2
    ), f"Expected at least 2 file-related tools, got {len(file_tools)}"

    # Search for "agent" tools
    agent_tools = registry.search_tools("agent")
    assert (
        len(agent_tools) >= 5
    ), f"Expected at least 5 agent-related tools, got {len(agent_tools)}"
