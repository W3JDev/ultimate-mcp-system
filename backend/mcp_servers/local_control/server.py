...
class LocalControlMCP:
    ...
    def list_processes(self):
        ...
        try:
            ...
            for p in processes:
                output += (
                    f"{p['PID']:<8} {p['Name']:<30} {p['CPU%']:<8} {p['Memory%']:<10}\n"
                )
    ...
    with gr.Blocks(theme=gr.themes.Soft(), title="Local Control MCP") as demo:
        ...
        **PC automation and system control**
        ...
        with gr.Tab("📟 System Commands"):
            ...
            cmd_btn.click(mcp.execute_command, inputs=[cmd_input], outputs=cmd_output)
        ...
        with gr.Tab("🖥️ App Control"):
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    list_btn.click(
                        mcp.list_processes, inputs=[], outputs=process_output
                    )
                ...
                with gr.Column():
                    ...
                    kill_btn.click(
                        mcp.kill_process, inputs=[kill_pid], outputs=kill_output
                    )
        ...
        with gr.Tab("📁 File Operations"):
            ...
            with gr.Tab("List Files"):
                ...
                dir_btn.click(mcp.list_files, inputs=[dir_input], outputs=dir_output)

            with gr.Tab("Read File"):
                file_input = gr.Textbox(
                    label="File Path", placeholder="~/documents/file.txt"
                )
                file_btn = gr.Button("Read File", variant="primary")
                file_output = gr.Textbox(label="Content", lines=15)
                file_btn.click(mcp.read_file, inputs=[file_input], outputs=file_output)
        ...
        with gr.Tab("⌨️ Input Control"):
            ...
            with gr.Tab("Type Text"):
                type_input = gr.Textbox(
                    label="Text to Type", placeholder="Hello, World!", lines=3
                )
                type_btn = gr.Button("Simulate Typing", variant="primary")
                type_output = gr.Textbox(label="Result")
                type_btn.click(
                    mcp.simulate_typing, inputs=[type_input], outputs=type_output
                )
            ...
            with gr.Tab("Press Key"):
                ...
                key_btn.click(mcp.press_key, inputs=[key_input], outputs=key_output)
        ...
        with gr.Tab("🌐 Browser Automation"):
            ...
            url_btn.click(mcp.open_url, inputs=[url_input], outputs=url_output)
        ...
        with gr.Tab("ℹ️ System Info"):
            ...
            info_btn.click(mcp.get_system_info, inputs=[], outputs=info_output)
