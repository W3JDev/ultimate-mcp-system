...
execute_command_tool = MCPTool(
    schema=ToolSchema(
        ...
        examples=["Execute: git status", "Run: python --version", "List files: ls -la"],
    ),
...
def list_processes_handler(
    limit: int = 20, sort_by: str = "memory"
) -> List[Dict[str, Any]]:
    """
...
kill_process_tool = MCPTool(
    schema=ToolSchema(
        ...
        examples=["Kill process 12345", "Terminate unresponsive application"],
    ),
...
def read_file_handler(filepath: str, max_size: int = 1000000) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"filepath": str(path), "size": size, "content": content}
    except Exception as e:
        return {"error": str(e)}
...
read_file_tool = MCPTool(
    schema=ToolSchema(
        ...
        examples=["Read config.json", "Show contents of README.md"],
    ),
...
open_url_tool = MCPTool(
    schema=ToolSchema(
        ...
        examples=["Open https://github.com", "Launch documentation website"],
    ),
...
get_system_info_tool = MCPTool(
    schema=ToolSchema(
        ...
        examples=["Show system information", "Get current resource usage"],
    ),
