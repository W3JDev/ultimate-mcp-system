...
class ToolSchema(BaseModel):
    ...
    examples: List[str] = Field(
        default_factory=list, description="Example usage strings"
    )
...
class MCPTool:
    ...
    def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "tool": self.schema.name, "data": result}
        except Exception as e:
            logger.error(f"❌ Tool {self.schema.name} failed: {e}")
            return {"success": False, "tool": self.schema.name, "error": str(e)}
