...
class TestCompleteWorkflow:
    ...
    def test_workflow_creation_flow(self):
        ...
        assert "workflow" in data.get("response", "").lower() or \
               "n8n" in data.get("server", "").lower()
    ...
    def test_agent_creation_flow(self):
        ...
        response = requests.post(
            f"{self.BASE_URL}/api/process",
            json={
                "message": "Create an AI agent for research tasks",
                "context": {}
            }
        )
        ...
        assert "agent" in data.get("response", "").lower() or \
               "agent_builder" in data.get("server", "").lower()
    ...
    def test_system_control_flow(self):
        ...
        response = requests.post(
            f"{self.BASE_URL}/api/process",
            json={
                "message": "Get system information",
                "context": {}
            }
        )
        ...
        assert "system" in data.get("response", "").lower() or \
               "local_control" in data.get("server", "").lower()
